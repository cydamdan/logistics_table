<template>
  ...
  <div class="btn">
    <nut-button title="Toast" shape="square" plain type="primary" @click="searchDate">搜索</nut-button>
  </div>
...
export default {
  ...
  setup(props) {
      ...
      if (popupStart.value && popupEnd.value) {
        try {
          ...
          const response = await axios.get(
            `http://localhost:3000/details?startDate=${startDate}&endDate=${endDate}`
          );
    ...
    onMounted(async () => {
      try {
        const response = await axios.get("http://localhost:3000/details"); // 使用server.js的地址
