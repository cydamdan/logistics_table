...
export default {
  ...
  setup(props) {
    ...
    const formatDateTo = (dateString) => {
      ...
      const second = dateString.substring(17);
    ...
    const searchDate = async () => {
      if (popupStart.value && popupEnd.value) {
        try {
          const startDate = formatDateTo(popupStart.value).substring();
          const endDate = formatDateTo(popupEnd.value);
          const response = await axios.get(
            `http://localhost:3000/recharge?startDate=${startDate}&endDate=${endDate}`
          );
    ...
    onMounted(async () => {
      try {
        const response = await axios.get("http://localhost:3000/recharge"); // 使用server.js的地址
        state.data2 = response.data; // 假设服务器返回的数据直接是表格所需的数组
      } catch (error) {
        console.error("Error fetching data:", error);
        // 根据需要，您可以在此添加错误处理，例如显示消息给用户
      }
    });
